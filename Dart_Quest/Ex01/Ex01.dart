...
class TimeCount {
  ...
  void time_out(int minuate, int second) {
    if (second != 0 || minuate != 0) {
      ...
      print(
          "${minuate.toString().padLeft(2, '0')}:${second.toString().padLeft(2, '0')}");
    } else {
      print(
          "${minuate.toString().padLeft(2, '0')}:${second.toString().padLeft(2, '0')}");
...
// 회고
// dart 언어의 첫 퀘스트를 진행 하였다. 작업을 진행하면서 너무 어려웠다.
// 민기님이 다 작업을 하셨다.. 배운 내용들이 아직 정리도 안되어있고,
// 실제로 적용을 하려고 하니 손에 잡히지 않았다. 
// 정리하고 이것저것 해보면서 손에 익혀야겠다.